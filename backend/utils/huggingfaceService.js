...
// Handles all AI interactions using open-source models via HF API:
...
// Default Models (adjustable based on HF Serverless API availability)
...
//    Sends image to a Vision-Language Model on HF.
// ============================================================
const identifyMedicineFromImage = async (imagePath) => {
  try {
    console.log(`🔍 Sending image to Hugging Face Vision API (${VISION_MODEL})...`);
    ...
    // Note: If chatCompletion with image_url fails on HF depending on model,
    // you could also fallback to hf.imageToText({ data: imageBuffer, model: ... })
    ...
    console.log(`✅ Hugging Face Vision identified: "${medicineName}"`);
    ...
    console.error("❌ Hugging Face Vision API Error:", error.message);
...
//    Sends medicine name to an Instruct Model on HF and returns JSON.
// ============================================================
...
-------------------------`;
    ...
    console.log(`🧠 Calling Hugging Face (${TEXT_MODEL}) to compile comprehensive JSON details...`);
...
${fdaContext}`;
    ...
    console.log("✅ Hugging Face medicine details received");
    ...
    try {
      ...
      console.error("❌ Failed to parse Hugging Face response as JSON");
    ...
    console.error("❌ Hugging Face Details Error:", error.message);
